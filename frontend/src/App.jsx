                    ...
                    <span
                      className={`px-3 py-1 rounded-full text-[11px] font-semibold border flex items-center space-x-1.5 shrink-0 ${
                        isEmergency
                          ...
                          : 'bg-amber-50 text-amber-750 text-amber-750 border-amber-200 animate-pulse'
                      }`}
                    >
                      {isPending ? (
                        <>
  ...
  const handleAddService = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/services`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: newServiceTitle.trim(),
          category: newServiceCategory,
          tag: newServiceBadge.trim(),
